...
public:
  ...
  Impl(size_type maxmem, float max_load_factor, Evictor* evictor, hash_func hasher) :
   maxmem_(maxmem), fifo_evictor(evictor), memused_(0), umap_(unordered_map<key_type,Element,hash_func>(0,hasher_)), max_load_factor_(max_load_factor), hasher_(hasher),lock_(PTHREAD_MUTEX_INITIALIZER) {
